...
**Rules:** Do NOT modify graph files. Keep briefs in natural language. Bias toward asking, not guessing.
...
**Rules:** Do NOT modify graph files. Ask questions sequentially. If the brief is already clear, suggest `/ygg.plan`.
...
- Do NOT create or modify graph files — only produce a plan document.
...
1. Follow the plan from `/ygg.plan` (or ask user what changes to make).
2. For each new node: create directory, `node.yaml`, `description.md`, additional artifacts.
3. For each modified node: update `node.yaml` and artifacts as specified.
4. If the plan specifies documentation or template updates, apply those too.
5. Run `ygg check` to validate.
